-- Phase 9 corrective migration: opportunity_documents has no sort_order column.
-- Preserve the applied migration and replace only the workspace function.
...
begin
  ...
  insert into public.application_checklist_items (application_id, opportunity_document_id, document_type, title, requirement_state, explanation, sort_order, due_at)
    select result_id, od.id, od.document_type, coalesce(nullif(od.original_wording,''), od.document_type), od.requirement_status, od.notes, 0, od.deadline_at
