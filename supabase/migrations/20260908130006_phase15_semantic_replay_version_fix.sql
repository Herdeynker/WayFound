-- Phase 15 corrective replay guard. A source monitor refresh updates last_seen_at,
-- but that operational timestamp is not a material opportunity change and must
-- not mint a new opportunity version or downstream match fingerprint.
...
comment on function public.phase4_capture_related_opportunity_version() is
  'Captures material related-record history while ignoring operational timestamp-only refreshes.';
